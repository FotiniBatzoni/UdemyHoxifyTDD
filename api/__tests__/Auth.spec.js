...
describe( 'Authentication', () => {
    ...
    it.each`
    language | message
    ${'gr'}    | ${gr.authentication_failure} 
    ${'en'}    | ${en.authentication_failure} 
    `('returns $message when authentication fails and language is set to $language', async ({ language, message }) => {
    ...
    it.each`
    language | message
    ${'gr'}    | ${gr.inactive_authentication_failure} 
    ${'en'}    | ${en.inactive_authentication_failure} 
    `('returns $message when authentication fails for inactive account and language is set to $language', async ({ language, message }) => {
        await addUser({...activeUser,inactive:true});
        const response = await postAuthentication({ email : 'user1@mail.com', password : 'P4ssword'},{language});
        expect(response.body.message).toBe(message);
      });
...
describe('Token Expiration' , () => {
    ...
    it('returns 403 when token is older than 1 week ', async() =>{
        ...
        const oneWeekAgo = new Date(Date.now() - (7*24*60*60*1000) -1 ) // (7*24*60*60*1000) 7-days 24-hours 60-minutes 60-seconds 1000-miliseconds
        await Token.create({
            token: token,
            userid: savedUser.id,
    ...
    it('refreshes lastUsedAt when unexpired token is used ', async() =>{
        ...
        expect(tokenInDb.lastUsedAt.getTime()).toBeLessThan(rightBeforeSendingRequest.getTime());
    });
